...
function aboutArrows(){
	...
	function check(id){
		var temp = parent.loc.frames[4].document.getElementById(id).src; 
		temp = temp.search(/none/);
		return temp
	};
...
function whereIAm(){
	var temp = parent.loc.frames[4].document.getElementById('coord').innerHTML;
	var coords = [];
	var x = temp.match(/\((.*?)\,/);
	var y = temp.match(/\,\s(.*?)\)/);
...
function step(){
	...
	if (arrows === 'getback'){setTimeout(getBack,500);}
		else{
			posibilities = [];
			for (var i=0;i<4;i++){
		...
		t1 = setTimeout(forward, 500,posibilities,rand);
		};
}
function pause(){
	pickUp();
